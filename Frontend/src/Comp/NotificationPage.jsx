...
// 🚨 Check Role on Mount
  useEffect(() => {
    ...
    const markAsSeen = async (id) => {
        try {
            await axios.put(`http://localhost:5000/api/notifications/${id}/seen`, {
                headers: {
                  'Authorization': `Bearer ${token}`,  // Attach the token in the Authorization header
                }
              });
        ...
        <div className="max-w-6xl mx-auto mt-8 p-6 bg-white rounded-xl shadow-lg">
            ...
            {filteredNotifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 bg-gray-50 rounded-xl">
                    ...
                    <p className="text-xl text-gray-500 font-medium">No notifications available</p>
                        ...
                        >
                            <div className="flex items-start">
                                <div className="flex-shrink-0">
                                    <div className={`p-3 rounded-full ${notif.seen ? 'bg-gray-100' : 'bg-blue-100'}`}>
                                        <MessageSquare className={`h-6 w-6 ${notif.seen ? 'text-gray-500' : 'text-blue-500'}`} />
                                    </div>
                                </div>
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex-shrink-0">
                                            {!notif.seen ? (
                                                ...
                                                <span className="flex items-center px-3 py-1.5 bg-green-100 text-green-800 text-sm font-medium rounded-lg">
                                    ...
                                    <div className="mt-3">
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div className="flex items-center text-gray-600">
                                                ...
                                                <span>To: <span className="font-medium">{notif.user?.username}</span> ({notif.user?.email})</span>
                                            </div>

                                            {notif.assignedBy && (
                                                <div className="flex items-center text-gray-600">
                                                    <User className="h-4 w-4 mr-1" />
                                                    <span>From: <span className="font-medium">{notif.assignedBy.username}</span> ({notif.assignedBy.email})</span>
                                                </div>
                                            )}
                                        </div>
